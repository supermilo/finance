...
# from helpers import apology, login_required, lookup, usd, local
...
def index():
    ...
    stocks = db.execute("SELECT symbol, SUM(shares) as shares, transactions FROM stocks WHERE userID = ? GROUP BY symbol HAVING (SUM(shares)) > 0;", session["user_id"])
    total_cash_stocks = 0
    for stock in stocks:
    ...
    return render_template("index.html", stocks=stocks, user_cash=user_cash[0], total_cash=total_cash, c="USD")
...
#         quote = lookup(stock["symbol"])
#         stock["name"] = quote["name"]
#         stock["price"] = quote["price"] * x
#         stock["total"] = stock["price"] * stock["shares"]



#     user_cash[0]["cash"] = user_cash[0]["cash"] * x
#     total_cash_stocks = (total_cash_stocks + stock["total"])

#     total_cash = (total_cash_stocks + user_cash[0]["cash"])
#     return render_template("currency.html", stocks=stocks, user_cash = user_cash[0], total_cash=total_cash, c=c)
...
def buy():
    ...
    if request.method == "POST":
        ...
        if user_cash < (shares_price):
            ...
            db.execute(
                "UPDATE users SET cash = cash - ? WHERE id = ?", shares_price, session["user_id"])
            db.execute("INSERT INTO stocks (userID, symbol, shares, price, transactions) VALUES (?, ?, ?, ?, ?)", session["user_id"], symbol.upper(), shares, price["price"],"buy")
...
def sell():
    ...
    if request.method == "POST":
        ...
        stocks = db.execute(
            "SELECT SUM(shares) as shares FROM stocks WHERE userID = ? AND symbol = ?;", session["user_id"], symbol,)[0]
        ...
        db.execute(
            "INSERT INTO stocks (userID, symbol, shares, price, transactions) VALUES (?, ?, ?, ?, ?)", session["user_id"], symbol.upper(),
            -shares, price,"sell")
